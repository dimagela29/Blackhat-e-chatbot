...
def prepare_ubuntu_data():
    ...
    if not dialog_dir.exists():
        print("Descompactando Dialogos...(Isso pode levar um tempo)")
        with tarfile.open(tarfile_path) as tar:
            tar.extractall(path = ubuntu_dir)
            print("Arquivo descompactao")
            
    #Donwload Metadata
    
    if not _ubuntu_meta_dir.joinpath('Trainfiles.csv').exists():
        print('Download de Metadatas...(Isso pode levar alguns minutos.)')
        for filename in ['trainfliles.csv', 'valfile.csv', 'testfiles.csv']:
            ...
            print(f"Download concluido com sucesso{csv_path}")
            
    print('Ubuntu preparando dados!')
...
with open(dialog_path, 'r', enconding = 'utf-8') as f:
#seguimos para o dialogo
    first_turn = True
    dialog = []
    users = []
