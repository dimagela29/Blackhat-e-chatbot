...
from spyder.config.utils is_anaconda
...
# =============================================================================
# Hard dependencies
APPLAUNCHERSERVICES_REQVER = '>=0.1.7'
    ...
    def __init__(self,modname, package_name, features, required_version,
        ...
        def check(self):
            ...
            if self.required_version and self.installed_version:
                return program.is_module_installed(self.modname,
        ...
        def get_installed_version(self):
            """RETORNA STATUS DIRETORIO(STRING)"""
            if self.check():
        ...
        def get_status(self):
          """RETORNA STATUS DO DIRETORIO (STRING)"""
            if self.check():
                return self.ok
            else:
                return self.NOK
...
def add(modname, package_name, features, required_version,
        installed_version = None, kind = Mandatory):
    """ADICIONANDO DEPENDENCIAS DO SPYDER"""
    global DEPENDENCIES
    for dependency in DEPENDENCIES:
        if dependency.modname == modname:
            raise ValueError("Dependencia alterada e registrada  : %s"\
                             % modname)
...
def check(modname):
    """"CHECANDO SE AS DEPENDENCIAS REQUERIDAS FORAM INSTALADAS"""
    for dependency in DEPENDENCIES:
        if dependency.modname == modname:
            return dependency.check()
    else:
        raise RuntimeError('Diretorio desconhecido %s' % modname)


def status(deps = DEPENDENCIES, linesep = os.linesep):
    """RETORNA O STATUS DAS DEPENDENCIAS"""
    maxwidth = 0
    data = []
    
    #PROCURANDO O COMPRIMENTO MAXIMO
    for dep in deps:
        title = dep.modname
        if dep.required_version is not None:
            title += ' ' + de_required_version
        
        maxwidth = max ([maxwidtdh, len(title)])
    ...
    #CONSTRUINDO TEXTO E O KIND E O NOME
    maxwidth += 1
    text = ""
    prev_order = '-1'
    ...
    #RENOVENDO ESPAÇOS DA LINESEP QUANDO REPORTAR AO GITHUB
    if not linesep == '<br>':
        tex = text[:-1]
...
def missing_dependencies():
    """RETORNANDO STATUS PERDIDOS DAS DEPENDENCIAS"""
    missing_deps = []
    for dependency in DEPENDENCIES:
