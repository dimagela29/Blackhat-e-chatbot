...
import troch.autogras as autograd
...
class Batchq:
    def _init_(self,config, val_config):
        self.config = config
        self.val_config = val_config
        
        #Carregando buffer da experiencia de replay do arquivo
        self.experience = replay_buffer.CsvReplayBuffer(
                config.experience_path, raw = config.raw_buffer,
                history_len = config.max_conversation_length, confiig = config,
                max_sentence_length = config.max_sentence_length,
                rewards = config.rewards, reward_weights = config.reward_weigth
        ...
        #checando todos os requerimentos carregados no buffer, não computar
    ...
    def q_update(self):
        """Carregando arquivos Q learning"""
        #sample a batch
        ...
        loss = loos_func(q_values, target_q_values)
        ...
        #clip gradients _Absolutamente crucial
        ...
        actions = to-var(torch.LongTensor(batch['action'])) #[tamanho do batch]
        
        #prepara as entradas do Q network
        ...
        #Gerando probabilidades para politica do network
    ...
    def get_target_q_values(self, batch, prior_rewards = None):
        rewards = to_var(torch.FloatTensor(batch['Rewards'])) #[tamanho do batch]
        not_done = to_var(torch.FloatTensor(1 - batch['done'])) #[Tamanho do batch]
        ...
        next_state_convs = [np.concatenate(
                (conv, blank_sentences)) for conv in batch[ next 'state']]
        ...
        if self.config.model not in VariationalModels:
            next_state_convs = [conv [:-1] for in next_state_convs]
            sext_state_lens =  np.concatenate(
                    [l[:-1]for l in next_state_lens])
        else:
            next_state_lens = np.concatenate([l for l in next_state_lens])
        next_con_len = to_var(torch.LongTensor(next_con_lens))

        mc_target_q_values = [[]] *self.config.monte_carlo_count
        for t in range(self.config.monte_carlo_count):
            #executa o Q network com saida 
