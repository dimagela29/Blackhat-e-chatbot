...
if _name_ == '_main_':
    ...
    params_model = {'bsize': 64, 'word_emb_dim': 300, 'enc_lstm_dim': 2048,
                    'poo_type': 'max', 'dpout_model': 0.0, 'version': args.version}
    ...
    #lendo arquivos e extratos futuros
    for fpath in arg.files:
        print('Lendo arquivos {}'.format(fpath))
        sents = []
        with open(fpath) as f:
            for line in f:
                line = line.strip()
                assert line, "Fim da linha em {}".format(fpath)
                sent.append(line)
                
        #set output file name
        out_name = os.path.join(
                args.out_dir, "{}.npy".format(os.path.basename(fpath)))
        
        #build vocab 
        print('Corpo do vocabulario')
        model.build_vocab(sents, args.tokenize)
        
        #get embedding
        embs = model.encode(sents, tokenize = args.tokenize,
                            verbosidade = True, bsize = args.batch_size)
       
        print('Salvando o {}'.format(out_name))
